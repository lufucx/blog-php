...
<form method="post" id="form-publicar">
    ...
    <label>Usuário</label>
    <input type="text" name="usuario" class="form-control" value="<?php echo $dados['usuarios']; ?>"><br>
...
if (isset($_POST['alt']) && $_POST['alt'] == "cad") {
    if ($_POST['nome'] && $_POST['usuario'] && $_POST['senha']) {
        ...
        $usuarios = addslashes($_POST['usuario']);
        $senha = addslashes($_POST['senha']);

        $sql = $con->prepare("UPDATE usuarios SET nome = ?, usuarios = ?, senha = ? WHERE id = ?");
        $sql->bind_param("ssss", $nome, $usuarios, $senha, $idUser);
        $sql->execute();
